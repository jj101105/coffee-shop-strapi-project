...
import { getOrderItemDTO } from '../../../utils/order-itemsHelpers';
...
export default factories.createCoreController(APICOLLECTION.ORDER_ITEM, () => ({
    ...
    async getAllOrderItem(ctx: any) {
        ...
        return createResponse({ 
            ctx, httpCode: HTTPCODE.SUCCESS, devCode: HTTPCODE.SUCCESS, message: 'Get all order items successfully', data: items });
    },
    ...
    async deleteOrderItem(ctx: any) {
        ...
        if (!item) throw new Error('Order item not found');
        return createResponse({ 
            ctx, httpCode: HTTPCODE.SUCCESS, devCode: HTTPCODE.SUCCESS, message: 'Delete order item successfully', data: item });
    },
}));
